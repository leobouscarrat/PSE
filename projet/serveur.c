...
>>>>>>> 71a9778a82e499a3a3a0e1157837155d085c88ae
...
=======
                ...
                printf("worker%d: génération de mot de passe.\n", data->tid);
                ecrireLog();
                sprintf(nom,"L'utilisateur %s a demandé un mot de passe aléatoire",utilisateurs[data->tid-1].pseudo);   //log
                nblus = ecrireLigne(journal, nom);
                if (nblus == -1) {
                    erreur_IO("ecrireLigne");
                }  
                ...
                sprintf(mes, "%s", motDePasse);                 // on prépare le motdepasse généré
                pthread_mutex_unlock(&mutex); /* On déverrouille le mutex */
                nbecr = ecrireLigne(data->canal, mes);          // envoie du motdepasse
>>>>>>> 71a9778a82e499a3a3a0e1157837155d085c88ae
                        if (nbecr == -1) {
                            erreur_IO("ecrireLigne");
                        } 
...
char *printTime(void){ // lafonction permet de retourner la date/heure sleon un format standard de time.h (utilité => log)
...
=======
    motDePasse[33]='\0';  // on ajoute le caractère de fin de chaine pour les printf et autres
>>>>>>> 71a9778a82e499a3a3a0e1157837155d085c88ae
}